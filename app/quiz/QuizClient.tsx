...
import { X, Lock, CheckCircle, AlertTriangle, ChevronRight, ChevronLeft, Shield, HelpCircle, Info, ArrowRight } from 'lucide-react';
            ...
            <div className="animate-fade-in">
              ...
              {step === 1 && (
                <div className="space-y-4">
                  ...
                  {['spouse', 'parent', 'child', 'other'].map(k => (
                    <OptionButton key={k} onClick={() => handleOption('relation', k)} icon={ChevronRight}>{t.quiz.options[k]}</OptionButton>
                  ))}
                </div>
              )}
              ...
              {step === 2 && (
                <div className="space-y-4">
                  ...
                  {['citizen', 'resident', 'unsure'].map(k => (
                    <OptionButton key={k} onClick={() => handleOption('status', k)} icon={ChevronRight}>{t.quiz.options[k]}</OptionButton>
                  ))}
                </div>
              )}
              ...
              {step === 6 && (
                <div className="space-y-4">
                  ...
                  {['yes', 'no', 'skip'].map(k => (
                    <OptionButton key={k} onClick={() => handleOption('arrests', k)} icon={ChevronRight}>{t.quiz.options[k]}</OptionButton>
                  ))}
                </div>
              )}
